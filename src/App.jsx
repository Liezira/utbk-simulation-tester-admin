...
import { Edit, Plus, Trash2, LogOut, Key, BarChart3, Filter } from 'lucide-react';
...
const UTBKAdminApp = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setScreen('dashboard'); loadTokens();
    } catch (error) { alert('Login Gagal. Cek Email/Pass.'); }
  };
  ...
  const createToken = async () => {
    ...
    try {
      await setDoc(doc(db, 'tokens', tokenCode), {
        tokenCode, studentName: newTokenName, studentPhone: newTokenPhone, status: 'active', createdAt: new Date().toISOString(),
      });
      alert(`Token Dibuat: ${tokenCode}`); setNewTokenName(''); setNewTokenPhone(''); loadTokens();
    } catch (error) { alert('Gagal.'); }
  };

  const loadTokens = async () => {
    const s = await getDocs(collection(db, 'tokens'));
    const t = []; s.forEach((d) => t.push(d.data()));
  ...
  const deleteAllTokens = async () => {
    if (!confirm("⚠️ PERINGATAN KERAS:\n\nHapus SEMUA data token? Data tidak bisa dikembalikan.")) return;
    try {
      const deletePromises = tokenList.map(t => deleteDoc(doc(db, "tokens", t.tokenCode)));
      await Promise.all(deletePromises);
      alert("Semua token berhasil dihapus."); loadTokens();
    } catch (error) { alert("Gagal menghapus sebagian data."); }
  };
  ...
  const deleteSoal = async (id) => {
    if(confirm('Hapus?')) await saveSoal(selectedSubtest, (bankSoal[selectedSubtest]||[]).filter(x => x.id !== id));
  };
  ...
  const generateDummy = async () => {
    ...
    for (const s of SUBTESTS) {
      ...
      if (need > 0) {
        const d = [];
        for (let i = 0; i < need; i++) d.push({ id: `d_${s.id}_${i}`, question: `Dummy ${s.name} ${i+1}`, image: '', options: ['A','B','C','D','E'], correct: 'A' });
        const fin = [...cur, ...d];
        ...
        {viewMode === 'tokens' ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow h-fit">
              ...
              <div className="space-y-4">
                ...
                <input value={newTokenPhone} onChange={e=>setNewTokenPhone(e.target.value)} className="w-full p-2 border rounded" placeholder="No HP"/>
                <button onClick={createToken} className="w-full bg-indigo-600 text-white py-2 rounded hover:bg-indigo-700 transition">Generate</button>
              </div>
            </div>

            {/* List Token & Statistik Interaktif */}
            <div className="md:col-span-2 space-y-4">
              
              {/* --- INFO CARD INTERAKTIF (KLIK UNTUK FILTER) --- */}
              ...
              <div className="bg-white p-6 rounded-xl shadow">
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-2">
                    <h2 className="font-bold text-lg">
                      {filterStatus === 'all' && 'Semua Token'}
                      {filterStatus === 'active' && 'Token Aktif'}
                      {filterStatus === 'used' && 'Token Terpakai'}
                      {filterStatus === 'expired' && 'Token Expired'}
                    </h2>
                    {filterStatus !== 'all' && (
                      <span className="text-xs bg-gray-200 px-2 py-1 rounded text-gray-600 cursor-pointer hover:bg-gray-300" onClick={() => setFilterStatus('all')}>(Reset Filter)</span>
                    )}
                  </div>
                  
                  <div className="flex gap-2">
                    <button onClick={loadTokens} className="text-indigo-600 text-sm hover:underline">Refresh</button>
                    {tokenList.length > 0 && (
                      <button onClick={deleteAllTokens} className="text-red-600 text-sm hover:underline font-bold ml-2">Hapus Semua</button>
                    )}
                  </div>
                </div>
                ...
                <div className="overflow-x-auto max-h-[500px] overflow-y-auto">
                  <table className="w-full text-sm text-left">
                    <thead className="bg-gray-50 sticky top-0"><tr><th className="p-2">Kode</th><th className="p-2">Nama</th><th className="p-2">Status</th><th className="p-2">Aksi</th></tr></thead>
                    <tbody>
                          ...
                          <tr key={t.tokenCode} className="border-b hover:bg-gray-50">
                            ...
                            <td className="p-2">
                              {expired ? 
                                ...
                                <span className={`px-2 py-1 rounded text-xs font-bold ${t.status==='active'?'bg-green-100 text-green-700':'bg-gray-200 text-gray-700'}`}>
                                  {t.status.toUpperCase()}
                                </span>
                              }
                            </td>
                            <td className="p-2"><button onClick={() => deleteToken(t.tokenCode)} className="text-red-500 hover:text-red-700"><Trash2 size={16}/></button></td>
                          </tr>
                        )
                      })}
                      {getFilteredList().length === 0 && (
                        <tr><td colSpan="4" className="p-8 text-center text-gray-400">Tidak ada data untuk filter ini.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
             <div className="flex justify-between items-center mb-6">
               <h2 className="text-xl font-bold">Editor Soal</h2>
               <button onClick={generateDummy} className="text-sm bg-green-100 text-green-700 px-4 py-2 rounded hover:bg-green-200">+ Auto Fill</button>
             </div>
             <div className="bg-gray-50 p-6 rounded-xl border mb-8">
               
               {/* --- UPDATE: FORMAT DROPDOWN (DIBUAT / DIBUTUHKAN) --- */}
               <select value={selectedSubtest} onChange={e => { setSelectedSubtest(e.target.value); resetForm(); }} className="w-full p-3 border rounded mb-4 bg-white font-medium">
                 {SUBTESTS.map(s => {
                    const created = bankSoal[s.id]?.length || 0;
                    const required = s.questions;
                    return (
                        <option key={s.id} value={s.id}>
                            {s.name} ({created} / {required})
                        </option>
                    );
                 })}
               </select>

               <textarea value={questionText} onChange={e => setQuestionText(e.target.value)} className="w-full p-3 border rounded mb-4" rows="3" placeholder="Pertanyaan..." />
               <input value={questionImage} onChange={e => setQuestionImage(e.target.value)} className="w-full p-3 border rounded mb-4" placeholder="URL Gambar (Opsional)" />
               <div className="space-y-2 mb-4">
                 {options.map((o, i) => (
                   <div key={i} className="flex gap-2 items-center">
                      <span className="w-8 h-8 flex items-center justify-center bg-indigo-100 font-bold rounded">{['A','B','C','D','E'][i]}</span>
                      <input value={o} onChange={e => {const n=[...options];n[i]=e.target.value;setOptions(n)}} className="w-full p-2 border rounded" placeholder={`Opsi ${['A','B','C','D','E'][i]}`} />
                   </div>
                 ))}
               </div>
               <div className="flex gap-2 mb-4">
                  {['A','B','C','D','E'].map(l => (
                    <button key={l} onClick={() => setCorrectAnswer(l)} className={`flex-1 py-2 border rounded font-bold ${correctAnswer===l?'bg-green-600 text-white':'bg-white'}`}>{l}</button>
                  ))}
               </div>
               <div className="flex gap-2">
                 <button onClick={addOrUpdate} className="flex-1 bg-indigo-600 text-white py-3 rounded font-bold">{editingId ? 'Simpan' : 'Tambah'}</button>
                 {editingId && <button onClick={resetForm} className="px-6 border py-3 rounded">Batal</button>}
               </div>
             </div>
             <div className="space-y-2 max-h-[500px] overflow-y-auto">
               {(bankSoal[selectedSubtest]||[]).map((q, i) => (
                 <div key={q.id} className="p-4 border rounded flex justify-between items-start">
                   <div className="flex-1 pr-4"><span className="font-bold text-indigo-600 mr-2">#{i+1}</span> {q.question}</div>
                   <div className="flex gap-2">
                      <button onClick={() => { setQuestionText(q.question); setQuestionImage(q.image||''); setOptions([...q.options]); setCorrectAnswer(q.correct); setEditingId(q.id); window.scrollTo({top:0, behavior:'smooth'}); }} className="text-indigo-600"><Edit size={18}/></button>
                      <button onClick={() => deleteSoal(q.id)} className="text-red-600"><Trash2 size={18}/></button>
                   </div>
                 </div>
               ))}
             </div>
