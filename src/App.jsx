...
const UTBKAdminApp = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      alert('Login Gagal. Cek Email/Pass.');
    }
  };
  ...
  const createToken = async () => {
    ...
    const tokenCode = `UTBK-${Math.random()
      .toString(36)
      .substring(2, 8)
      .toUpperCase()}`;
    try {
  ...
  const deleteAllTokens = async () => {
    if (
      !confirm(
        '⚠️ PERINGATAN KERAS:\n\nHapus SEMUA data token? Data tidak bisa dikembalikan.'
      )
    )
      return;
    try {
      const deletePromises = tokenList.map((t) =>
        deleteDoc(doc(db, 'tokens', t.tokenCode))
      );
      await Promise.all(deletePromises);
      alert('Semua token berhasil dihapus.');
      loadTokens();
    } catch (error) {
  ...
  // --- CALCULATE COUNTS ---
  const activeTokens = tokenList.filter(
    (t) => t.status === 'active' && !isExpired(t.createdAt)
  );
  ...
  const deleteSoal = async (id) => {
    if (confirm('Hapus?'))
      await saveSoal(
        selectedSubtest,
        (bankSoal[selectedSubtest] || []).filter((x) => x.id !== id)
      );
  };
              ...
              <div className="bg-white p-6 rounded-xl shadow">
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    {filterStatus !== 'all' && (
                      <span
                        className="text-xs bg-gray-200 px-2 py-1 rounded text-gray-600 cursor-pointer hover:bg-gray-300"
                        onClick={() => setFilterStatus('all')}
                      >
                        (Reset Filter)
                      </span>
                  ...
                  <div className="flex gap-2">
                    ...
                    {tokenList.length > 0 && (
                      <button
                        ...
                        className="text-red-600 text-sm hover:underline font-bold ml-2"
                      >
                        Hapus Semua
                      </button>
                    )}
                    ...
                    <tbody>
                      {getFilteredList().map((t) => {
                        ...
                        return (
                          ...
                          >
                            <td className="p-2 font-mono text-indigo-600 font-bold">
                              {t.tokenCode}
                            </td>
                            <td className="p-2">
                              {t.studentName}
            ...
            <div className="bg-gray-50 p-6 rounded-xl border mb-8">
              ...
              <div className="space-y-2 mb-4">
                {options.map((o, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    <span className="w-8 h-8 flex items-center justify-center bg-indigo-100 font-bold rounded">
                      {['A', 'B', 'C', 'D', 'E'][i]}
                    </span>
                    <input
                      value={o}
              ...
              <div className="flex gap-2 mb-4">
                {['A', 'B', 'C', 'D', 'E'].map((l) => (
                  <button
                    ...
                    className={`flex-1 py-2 border rounded font-bold ${
                      correctAnswer === l
              ...
              <div className="flex gap-2">
                ...
                {editingId && (
                  <button
                    onClick={resetForm}
                    className="px-6 border py-3 rounded"
                  >
                    Batal
                  </button>
                )}
