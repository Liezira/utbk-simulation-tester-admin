        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-in fade-in zoom-in duration-200">
          ...
          <div className="flex-1 overflow-y-auto p-0">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y">
                {visibleUsers.map(u => (
                  <tr key={u.id} className={`hover:bg-green-50 transition cursor-pointer ${selectedUserIds.includes(u.id) ? 'bg-green-50' : ''}`}
                      onClick={() => toggleUserSelection(u.id)}>
                    <td className="p-4 text-center">
                      <input type="checkbox" checked={selectedUserIds.includes(u.id)} onChange={() => {}} className="w-4 h-4 rounded cursor-pointer"/>
                    </td>
                    <td className="p-4">
